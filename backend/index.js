...
// PostgreSQL pool (mysql2-сумісний інтерфейс)
const pool = require('./db');
const { ragSearch } = require('./assistant');

// ── Ініціалізація PostgreSQL та створення адміна ─────
(async () => {
  try {
    const [rows] = await pool.query("SELECT 1");
    log('✅ PostgreSQL (pgvector) — з\'єднання встановлено');

    // Перевіряємо наявність адміна
    ...
    if (parseInt(admins[0].count) === 0) {
      ...
      await pool.query(
        "INSERT INTO users (full_name, email, password_hash, role, access_level, is_active) VALUES (?, ?, ?, 'admin', 'Secret', TRUE)",
        ['Адміністратор', 'admin@mitit.edu.ua', hash]
      );
      log('✅ Адміністратора створено: admin@mitit.edu.ua / qwerty123');
    }
  } catch (err) {
    console.error('❌ PostgreSQL — помилка з\'єднання:', err.message);
  }
})();
...
app.post('/auth/login', loginLimiter, async (req, res) => {
  try {
    ...
    const [users] = await pool.query('SELECT * FROM users WHERE email = ? AND (is_active = TRUE OR is_active IS NULL)', [cleanEmail]);
  ...
  try {
    const [rows] = await pool.query(`
      SELECT t.*, s.file_type, s.security_stamp,
             ...
             (SELECT json_agg(json_build_object('title', tr.source_name, 'url', tr.source_url)) FROM term_references tr WHERE tr.term_id = t.id) as refs
      FROM favorites uf
...
app.post('/favorites/:termId', async (req, res) => {
  try {
    await pool.query('INSERT INTO favorites (user_id, term_id) VALUES (?, ?) ON CONFLICT DO NOTHING', [req.user.id, req.params.termId]);
...
app.post('/favorite-docs/:id', async (req, res) => {
  try {
    await pool.query('INSERT INTO favorite_documents (user_id, source_id) VALUES (?, ?) ON CONFLICT DO NOTHING', [req.user.id, req.params.id]);
...
app.put('/favorite-docs/:id/note', async (req, res) => {
  try {
    ...
    if (!note || !note.trim()) {
      ...
      await pool.query(
        'INSERT INTO document_notes (user_id, source_id, note) VALUES (?, ?, ?) ON CONFLICT (user_id, source_id) DO UPDATE SET note = EXCLUDED.note, updated_at = NOW()',
        [req.user.id, req.params.id, note.trim()]
      );
    }
...
app.get('/users', requireRole('admin'), async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT id, full_name AS name, email, role, access_level AS clearance, CASE WHEN is_active THEN 'Активний' ELSE 'Заблокований' END AS status FROM users");
    res.json(rows);
  } catch (err) {
    console.error(err);
...
app.post('/users', requireRole('admin'), async (req, res) => {
  try {
    ...
    if (err.code === '23505') return res.status(400).json({ error: 'Користувач з таким Email вже існує' });
...
app.put('/users/:id', requireRole('admin'), async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [role, clearance, status === 'Активний', id]
    );
...
app.get('/stats', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT t.category,
              COUNT(*) as total,
              SUM(CASE WHEN t.is_actual IS NOT FALSE THEN 1 ELSE 0 END) AS actual,
              SUM(CASE WHEN t.definition_source_type = 'AI-Generated' THEN 1 ELSE 0 END) AS ai_generated
       FROM terms t
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        // ── Зберігаємо чернетки в БД — bulk INSERT у транзакції ──────────────
        await sendSSE(93, `Збереження ${terms.length} термінів...`);
        await pool.query('DELETE FROM draft_terms WHERE source_id = ?', [sourceId]);

        if (terms.length > 0) {
          // Будуємо один bulk INSERT замість N окремих запитів
          const { pool: pgPool } = require('./db');
          const vals   = [];
          const params = [];
          let   pi     = 1;
          for (const t of terms) {
            vals.push(`($${pi},$${pi+1},$${pi+2},$${pi+3},$${pi+4},$${pi+5},$${pi+6},$${pi+7})`);
            params.push(
              sourceId,
              (t.term || '').trim(),
              ...
              t.definition_source_type || 'Document',
        ...
        // OSINT-збагачення вимкнено (Wikipedia + wikiAgent) — пришвидшує обробку
  ...
  try {
    ...
    if (Array.isArray(bodyTerms) && bodyTerms.length > 0) {
      ...
      termsToConfirm = drafts.map(d => ({
        ...
        references: (() => { try { return d.references ? JSON.parse(d.references) : []; } catch { return []; } })(),
      }));
    }
    ...
    // ── BULK-операції у транзакції (замість N×7 окремих запитів) ──────────────
    // PostgreSQL без транзакції робить fsync на кожен INSERT → з 100 термінів
    // це ~100 fsyncs. Транзакція → 1 fsync при COMMIT. Прискорення 10-100×.
    const conn = await pool.getConnection();
    try {
      await conn.query('BEGIN');

      // 1. Знаходимо всі вже існуючі терміни ОДНИМ запитом
      const lowerNames = uniqueTerms.map(t => t._cleanName.toLowerCase());
      const [existing] = await conn.query(
        `SELECT id FROM terms WHERE LOWER(term_name) = ANY(?)`,
        [lowerNames]
      );

      // 2. Bulk-видалення пов'язаних записів (4 запити замість N×4)
      if (existing.length > 0) {
        const ids = existing.map(r => r.id);
        const ph  = ids.map(() => '?').join(',');
        await conn.query(`DELETE FROM term_embeddings WHERE term_id IN (${ph})`, ids);
        await conn.query(`DELETE FROM term_references  WHERE term_id IN (${ph})`, ids);
        await conn.query(`DELETE FROM favorites         WHERE term_id IN (${ph})`, ids);
        await conn.query(`DELETE FROM terms             WHERE id       IN (${ph})`, ids);
      }

      // 3. INSERT термінів у транзакції — без окремих fsyncs на кожен рядок
      const allRefs = [];
      for (const term of uniqueTerms) {
        ...
        embeddingQueue.push({ termId, termName, definition });

        if (term.references?.length > 0) {
          for (const ref of term.references) {
            allRefs.push([termId, ref.title || 'OSINT', ref.url]);
          }
        }
      }

      // 4. Bulk INSERT references — один запит на всі посилання
      if (allRefs.length > 0) {
        const refVals   = [];
        const refParams = [];
        let pi = 1;
        for (const [tid, name, url] of allRefs) {
          refVals.push(`($${pi}, $${pi + 1}, $${pi + 2})`);
          refParams.push(tid, name, url);
          pi += 3;
        }
        await conn._client.query(
          `INSERT INTO term_references (term_id, source_name, source_url) VALUES ${refVals.join(',')}`,
          refParams
        );
      }

      await conn.query('DELETE FROM draft_terms WHERE source_id = ?', [sourceId]);
      await conn.query("UPDATE sources SET processing_status='confirmed' WHERE id=?", [sourceId]);
    ...
    // Відповідаємо одразу — ембединги генеруємо у фоні (не блокуємо відповідь)
    res.json({ message: 'Terms confirmed and added', count: termsToConfirm.length });

    // ── Фонова генерація ембедингів (fire-and-forget) ──────────────────────
    // Не блокує відповідь. Якщо Ollama недоступна — просто пропустить.
    setImmediate(async () => {
      console.log(`[Embed] Фонова генерація для ${embeddingQueue.length} термінів...`);
      const BATCH = 3; // по 3 паралельно щоб не перевантажувати Ollama
      for (let i = 0; i < embeddingQueue.length; i += BATCH) {
        const batch = embeddingQueue.slice(i, i + BATCH);
        await Promise.allSettled(batch.map(({ termId, termName, definition }) =>
          addTermEmbedding(termId, termName, definition)
        ));
      }
      console.log(`[Embed] Фонову генерацію завершено`);
    });
  } catch (error) {
...
// ── Регенерація векторних ембедингів (pgvector) ───────────────────────────
// Запускається адміном після міграції або повного перезавантаження.
// POST /regenerate-embeddings → { processed, failed, total }
app.post('/regenerate-embeddings', requireRole('admin'), async (req, res) => {
  try {
    const { limit = 500 } = req.body || {};
    // Терміни без ембедингів
    const [rows] = await pool.query(
      `SELECT t.id, t.term_name, t.definition
       FROM terms t
       LEFT JOIN term_embeddings te ON te.term_id = t.id
       WHERE te.id IS NULL AND t.is_actual = TRUE
       LIMIT ?`,
      [parseInt(limit)]
    );

    if (rows.length === 0) return res.json({ message: 'Всі ембединги актуальні', processed: 0 });

    res.json({ message: `Регенерацію розпочато для ${rows.length} термінів (фоново)`, total: rows.length });

    // Фонова обробка пакетами по 10
    setImmediate(async () => {
      ...
      for (let i = 0; i < rows.length; i += BATCH) {
        ...
        await Promise.all(batch.map(async t => {
          try {
            await addTermEmbedding(t.id, t.term_name, t.definition);
            processed++;
          } catch (e) {
            failed++;
          }
        }));
        if (i % 50 === 0) console.log(`[Embed] ${processed}/${rows.length} done, ${failed} failed`);
      }
      console.log(`[Embed] Регенерацію завершено: ${processed} OK, ${failed} ERR`);
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/assistant', authMiddleware, async (req, res) => {
  try {
    ...
    if (!query || query.trim().length < 3) {
      return res.status(400).json({ error: 'Запит занадто короткий (мінімум 3 символи)' });
    }

    console.log(`[assistant] запит від ${req.user?.email || 'unknown'}: "${query.trim().slice(0, 60)}"`);

    // Зберігаємо в історію (fire-and-forget)
    if (req.user?.id) {
      pool.query(
        'INSERT INTO search_history (user_id, query_text, type) VALUES (?, ?, ?)',
        [req.user.id, query.trim(), 'Асистент']
      ).catch(() => {});
    }

    const result = await ragSearch(query.trim(), req.user, 8);
    res.json(result);
  } catch (error) {
    console.error('[assistant] ПОМИЛКА:', error);
    res.status(500).json({ error: `Помилка асистента: ${error.message}` });
  }
});

// ── Trigram пошук (3-грами, окремий endpoint) ─────────────────────────────
app.get('/trigram-search', async (req, res) => {
  try {
    const { q, threshold = 0.2, limit = 20 } = req.query;
    if (!q || q.length < 3) return res.json([]);

    const allowed     = getAllowedStamps(req.user?.access_level || 'Public');
    const stampPh     = allowed.map(() => '?').join(',');

    const [rows] = await pool.query(
      `SELECT
         t.id, t.term_name, t.definition, t.category,
         t.source_id, s.file_name AS source_file_name,
         s.title AS source_title, s.doc_type AS source_doc_type,
         GREATEST(
       ...
       FROM terms t
       LEFT JOIN sources s ON t.source_id = s.id
       WHERE s.security_stamp IN (${stampPh})
         AND t.is_actual = TRUE
         AND (
           similarity(t.term_name, ?) > ?
           OR similarity(t.definition, ?) > ?
         )
       ...
       LIMIT ?`,
      [q, q, ...allowed, q, threshold, q, parseFloat(threshold) - 0.05, parseInt(limit)]
    );

    res.json(rows);
  } catch (error) {
    console.error('[trigram-search]', error);
    res.status(500).json({ error: 'Trigram search failed' });
  }
});
